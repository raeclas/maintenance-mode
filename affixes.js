...
export const AFFIXES = {
  atkFlat: { lane: "atk",   kind: "flat", base: 6,    per: 8,    round: true,  label: v => `+${v} ATK` },
...
function affixValue(a, tier, rng) {
  ...
  const v = mid * (0.75 + 0.5 * rng()); // ±25% roll variance — the value chase
...
export function rollAffixes(ip, n, rng = Math.random) {
  ...
  return pool.slice(0, Math.min(n, pool.length))
    .map(id => ({ id, tier, value: affixValue(AFFIXES[id], tier, rng) }));
}
